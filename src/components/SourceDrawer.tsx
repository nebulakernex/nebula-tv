...
export const SourceDrawer: React.FC<SourceDrawerProps> = ({
  ...
  const enabledProviders: CloudstreamPlugin[] = (settings.cloudstreamRepo?.plugins ?? []).filter(
    (provider: CloudstreamPlugin) => provider.enabled
  );
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between pb-4 border-b border-white/10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-black uppercase tracking-tight text-base text-white">Stream Sources</h3>
              <p className="text-xs text-zinc-400">Provider diagnostics</p>
            </div>
          </div>
        ...
        <button type="button" onClick={onRefreshApi} className="w-full py-3 px-4 rounded-lg bg-[#7000FF] hover:bg-[#8222FF] text-white text-xs font-bold uppercase tracking-widest flex items-center justify-center gap-2">
          <RefreshCw className="w-3.5 h-3.5" />
          Refresh Provider Metadata
        </button>

        <div className="space-y-3">
          <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest block">Enabled Providers ({enabledProviders.length})</span>
              ...
              enabledProviders.map(provider => (
                <div key={provider.internalName} className="p-3.5 rounded-xl bg-[#050505] border border-white/10 space-y-1">
                  <div className="flex items-center justify-between text-xs font-bold text-zinc-200">
                    ...
                    <span className="text-green-400 text-[10px] font-mono">{provider.adapterAvailable ? 'ADAPTER' : 'METADATA'}</span>
                  </div>
